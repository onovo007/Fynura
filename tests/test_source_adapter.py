...
def test_fixture_extracts_without_network():
    c = SourceCandidate(
        ...
        publisher="CDC",
        threat_id="measles",
        geography=Geography(name="United States", level="country"),
        source_type="national_health_authority",
        retrieved_at=datetime.now(UTC),
    )
    result = CDCMeaslesAdapter().extract(
        c, "As of August 20, 2026, a total of 1,234 measles cases were reported.", "run"
    )

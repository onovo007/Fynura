...
def observation(value=10, source="who", cutoff=date(2026, 8, 20), excerpt="fixture"):
    return Observation(
        threat_id="ebola", indicator="confirmed_cases", value=value, unit="persons",
        geography=Geography(name="DRC", level="country", code="COD"),
        reporting_period_end=cutoff, publication_date=cutoff,
        retrieved_at=datetime(2026, 8, 29, tzinfo=UTC), source_id=source,
        source_url=f"https://example.org/{source}", source_type="international_health_authority",
        case_definition="confirmed", extraction_method="fixture", extraction_confidence=.99,
        supporting_excerpt=excerpt, run_id="test",
    )
...
def test_independent_agreement_improves_confidence():
    assert score([observation(source="who"), observation(source="national")]) > score([observation()])

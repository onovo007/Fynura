...
def obs(value, indicator="confirmed_cases"):
    return Observation(threat_id="ebola", indicator=indicator, value=value, unit="persons", geography=Geography(name="Example", level="country"), event_date=date(2026, 1, 1), source_id="who", source_url="https://who.int/example", source_type="international_health_authority", extraction_method="fixture", extraction_confidence=1, supporting_excerpt="Expert annotated fixture", run_id="test")
def test_changes_retain_inputs():
    ...
    assert absolute_change(new, old).input_observation_ids == [old.observation_id, new.observation_id]
